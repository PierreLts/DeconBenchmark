...
## 1. Installations
First intall libraries and packages running `install_r_lib.sh`
...
Running `merge_batches.sh` will save "merged_batches.rds" which are the 4 batches of your choice merged (in our case the 4 batches "GFB...Seurat.rds")

### 2.2. Data Generation
Run `all_data.sh`. You might want to run it twice because of a dependency issue: `GT_generation.sh` uses 
